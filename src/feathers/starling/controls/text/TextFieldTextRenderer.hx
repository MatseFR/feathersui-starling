...
import feathers.starling.controls.text.BaseTextRenderer;
import feathers.starling.utils.geom.GeomUtils;
import feathers.starling.utils.math.MathUtils;
import feathers.starling.utils.texture.TextureUtils;
#if flash
 ...
 */
class TextFieldTextRenderer extends BaseTextRenderer implements ITextRenderer
{
	/**
	   @private
	**/
	...
	**/
	 ...
	 * @see http://help.adobe.com/en_US/FlashPlatform/reference/actionscript/3/flash/text/TextFormat.html flash.text.TextFormat
	 */
	public var selectedTextFormat(get, set):openfl.text.TextFormat;
	private var _selectedTextFormat:openfl.text.TextFormat;
	private function get_selectedTextFormat():openfl.text.TextFormat { return this._selectedTextFormat; }
	private function set_selectedTextFormat(value:openfl.text.TextFormat):openfl.text.TextFormat
	{
	...
	private function get_maxTextureDimensions():Int { return this._maxTextureDimensions; }
	...
	private function get_nativeFilters():Array<BitmapFilter> { return this._nativeFilters; }
	...
	**/
	...
	/**
	   @private
	**/
	private var _lastContentScaleFactor:Float = 0;
	...
	**/
	private function createTextureOnRestoreCallback(snapshot:Image):Void
	{
		...
		texture.root.onRestore = function(tex:ConcreteTexture):Void
		{
			...
			var offsetY:Float = self.getVerticalAlignOffsetY();
	...
	**/
	...
	/**
	   @private
	**/
	private function calculateSnapshotDimensions():Void
	{
		var starling:Starling = this.stage != null ? this.stage.starling : Starling.current;
		var scaleFactor:Float = starling.contentScaleFactor;
		...
		var rectangleSnapshotWidth:Float = Math.fceil(this.textField.width);
		var rectangleSnapshotHeight:Float = Math.fceil(this.textField.height);
		if (this._updateSnapshotOnScaleChange)
		{
			this.getTransformationMatrix(this.stage, HELPER_MATRIX);
	...
	**/
	private function refreshSnapshot():Void
	{
		...
		if (this.textSnapshots != null)
		{
			...
			{
				this.textSnapshots.resize(snapshotIndex);
			}
		}
		if (this._updateSnapshotOnScaleChange)
		{
